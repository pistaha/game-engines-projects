...
public class RedSlimeAmbushRecipeEffect : RecipeEffectBase
{
    ...
    private GameObject CreateSlime(Vector3 spawnPosition)
    {
        if (slimePrefab != null)
        {
            ...
            {
                Debug.LogWarning("RedSlimeAmbushRecipeEffect: slimePrefab поврежден, создан запасной слизень. " + exception.Message);
            }
        }

        GameObject slime = GameObject.CreatePrimitive(PrimitiveType.Sphere);
        slime.name = "RedSlimeFallback";
